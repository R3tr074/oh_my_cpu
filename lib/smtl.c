...
void smtl_init(smtl_handle *psh, int num_threads) {
  int err = 0;

  struct smtl_t *sh = (struct smtl_t *)malloc(sizeof(struct smtl_t));
  if (sh == NULL) {
    fprintf(stderr, "ERROR: smtl_init allocation failed.\n");
    exit(0);
  }
  ...
  memset(sh->task_queues, 0, num_threads * sizeof(struct queue_node_t *));
  ...
  for (i = 0; i < num_threads; i++) {
    ...
    err = pthread_create(sh->tids + i, NULL, smtl_thread_func, stp);
    if (err != 0) {
...
void smtl_add_task(smtl_handle sh, task_func_t task_func, void *params) {
